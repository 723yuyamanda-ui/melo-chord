...
export default function SimpleKeyboardInput() {
  ...
  return (
    <div className="h-full w-full flex flex-col bg-gray-950 text-white select-none relative overflow-y-auto scrollbar-none px-4 lg:px-6">
      ...
      {/* ヘッダー */}
      <div className="border-b border-white/10 flex items-center justify-between py-2 shrink-0">
        ...
        <div className="flex bg-gray-900/60 backdrop-blur-xl p-1 rounded-2xl border border-white/10 shadow-inner">
          <button onClick={() => setSelectedBars(4)} className={`px-3 py-1 rounded-xl text-xs font-black transition-all ${selectedBars === 4 ? 'bg-teal-500 text-gray-950 shadow-md' : 'text-gray-400 hover:text-white'}`}>4小節</button>
          <button onClick={() => setSelectedBars(8)} className={`px-3 py-1 rounded-xl text-xs font-black transition-all ${selectedBars === 8 ? 'bg-teal-500 text-gray-950 shadow-md' : 'text-gray-400 hover:text-white'}`}>8小節</button>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setIsListOpen(true)} className="flex flex-col items-center justify-center gap-0.5 text-teal-400 hover:text-teal-300 active:scale-95 transition-all relative lg:hidden">
            <div className="w-7 h-7 flex items-center justify-center bg-white/10 rounded-xl hover:bg-white/20 transition-colors"><FolderHeart size={13} /></div>
      ...
      <div className="flex-1 flex flex-col justify-between gap-2.5 pt-2 pb-4">
        ...
        {/* タイムライン */}
        <div className="w-full bg-gray-900/60 backdrop-blur-2xl border border-white/10 p-2.5 rounded-2xl h-[128px] lg:h-[135px] flex flex-col relative shrink-0 shadow-xl">
          <div className="flex justify-between items-center mb-1">
            <div className="flex items-center gap-2 overflow-hidden">
              ...
              <span className="text-[9px] font-mono font-black text-teal-400 bg-gray-950/80 px-2 py-0.5 rounded-full border border-teal-500/30">
            ...
            <div className="flex items-center gap-2">
              {/* 全体ずらし（ナッジ） */}
              <div className="flex items-center gap-1 bg-gray-950/80 p-0.5 rounded-xl border border-white/10">
              ...
              <select value={startBarSelection} onChange={(e) => setStartBarSelection(Number(e.target.value))} className="bg-gray-950/80 text-teal-400 font-mono font-black text-xs py-0.5 px-1.5 rounded-xl border border-white/10 focus:outline-none">
            ...
            {Array.from({ length: 144 }).map((_, stepCol) => {
              ...
              return (
                <div key={stepCol} className={`flex-shrink-0 relative ${isMeasureStart ? 'ml-1.5 border-l-2 border-dashed border-gray-700 pl-1' : ''}`}>
                  {isMeasureStart && (
                    <span className={`absolute -top-5 left-0.5 text-[8px] font-mono font-bold ${isDisabledStep ? 'text-gray-800' : isPickupArea ? 'text-purple-400' : 'text-gray-400'}`}>
                  ...
                  <div 
                    ...
                    isDisabledStep ? 'bg-gray-950/40 opacity-20 cursor-not-allowed' 
                    : isNowPlaying ? 'bg-amber-400 text-black scale-110 z-20 shadow-[0_0_15px_rgba(250,204,21,0.6)]' 
                    : isSelectedStep ? 'bg-gray-950 border-2 border-teal-400 text-teal-400 animate-pulse shadow-[0_0_10px_rgba(45,212,191,0.4)]' 
                    : found ? (isPickupArea ? 'bg-purple-600/30 border border-purple-500/50 text-purple-200' : 'bg-blue-600/30 border border-blue-500/50 text-blue-200')
                    : 'bg-gray-950/60 border border-white/5 text-gray-700 hover:border-gray-600'
                  }`}>
        ...
        {/* ステップ操作バー */}
        <div className="w-full shrink-0 grid grid-cols-4 gap-2 p-2 bg-gray-900/60 backdrop-blur-2xl border border-white/10 rounded-2xl shadow-xl">
          <div className="flex bg-gray-950/80 p-0.5 rounded-xl border border-white/5">
            <button onClick={() => setGridMode('8n')} className={`flex-1 py-1 text-[9px] font-black rounded-lg transition-all ${gridMode === '8n' ? 'bg-blue-600 text-white' : 'text-gray-500'}`}>8分</button>
            <button onClick={() => setGridMode('16n')} className={`flex-1 py-1 text-[9px] font-black rounded-lg transition-all ${gridMode === '16n' ? 'bg-blue-600 text-white' : 'text-gray-500'}`}>16分</button>
          </div>

          <button onClick={handleSkip} disabled={isPlaying || !isRecordMode} className="flex flex-col items-center justify-center bg-gray-950/80 hover:bg-white/5 border border-white/10 rounded-xl py-1 text-blue-400 active:scale-95 disabled:opacity-20 transition-all">
            <FastForward size={14} />
            <span className="text-[8px] font-black mt-0.5 text-gray-300">休符</span>
          </button>

          <button onClick={handleClearLast} disabled={isPlaying || melodyGrid.length === 0} className="flex flex-col items-center justify-center bg-gray-950/80 hover:bg-white/5 border border-white/10 rounded-xl py-1 text-red-400 active:scale-95 disabled:opacity-20 transition-all">
            <Trash2 size={14} />
            <span className="text-[8px] font-black mt-0.5 text-gray-300">消去/戻る</span>
          </button>

          <button onClick={toggleBpm} className="flex flex-col items-center justify-center bg-gray-950/80 hover:bg-white/5 border border-white/10 rounded-xl py-1 text-amber-400 active:scale-95 transition-all">
            <Clock size={14} />
            <span className="text-[8px] font-mono font-black mt-0.5 text-amber-400">{bpm}</span>
          </button>
        </div>
        ...
        <div className="flex flex-col justify-center">
          <div className="flex justify-between items-center px-1 mb-2">
            <div className="flex bg-gray-950/80 p-1 rounded-xl border border-white/10">
               <button onClick={() => setIsRecordMode(false)} className={`px-3 py-1 rounded-lg text-[10px] font-black flex items-center gap-1 transition-all ${!isRecordMode ? 'bg-white/10 text-teal-400' : 'text-gray-500'}`}>
                 <Volume2 size={12} />
                 <span>練習</span>
               </button>
               <button onClick={() => setIsRecordMode(true)} className={`px-3 py-1 rounded-lg text-[10px] font-black flex items-center gap-1 transition-all ${isRecordMode ? 'bg-red-600 text-white shadow-md' : 'text-gray-500'}`}>
                 <div className="w-2 h-2 rounded-full bg-current" />
                 <span>入力</span>
               </button>
            </div>

            <button onClick={handleTogglePreview} className="px-5 py-1.5 bg-blue-600 hover:bg-blue-500 text-white text-[10px] font-black rounded-xl flex items-center gap-1.5 shadow-lg active:scale-95 transition-all">
          ...
          {/* スライド鍵盤 */}
          <div ref={(el) => { if (el && el.scrollLeft === 0) el.scrollLeft = 448; }} className="w-full relative h-[140px] lg:h-[150px] bg-gray-950/90 flex overflow-x-auto rounded-2xl border border-white/10 scrollbar-none px-2 shadow-2xl">
            <div className="flex relative h-full" style={{ width: '1408px' }}>
              {SLIDABLE_PIANO_KEYS.map((key, index) => (
                <div key={index} className="w-[64px] h-full relative shrink-0">
                  <button onClick={() => handleInputNote(key.offset, key.octave)} disabled={isPlaying} className="w-full h-[96%] bg-gray-100 hover:bg-blue-50 active:bg-blue-100 border-r border-gray-300 rounded-b-2xl flex flex-col items-center justify-end pb-4 text-gray-950 font-black text-xs border-t-4 border-blue-400/40 transition-colors">
                  ...
                  {key.hasBlack && (
                    <button onClick={() => handleInputNote(key.bOffset!, key.octave)} disabled={isPlaying} className="absolute top-0 right-[-16px] w-[32px] h-[55%] bg-gray-900 hover:bg-gray-800 active:bg-blue-600 rounded-b-xl text-white font-bold text-[9px] flex items-end justify-center pb-2 border border-black z-10 shadow-md transition-colors">{key.bLabel}</button>
                  )}
                </div>
              ))}
            </div>
        ...
        <div className="shrink-0 flex flex-col items-center gap-1.5 pt-1">
          ...
          <button onClick={handleNavigateToSuggest} disabled={melodyGrid.length === 0 || isPlaying} className="w-full h-12 lg:h-13 bg-gradient-to-r from-teal-500 to-blue-600 hover:from-teal-400 hover:to-blue-500 disabled:opacity-20 rounded-2xl font-black text-xs lg:text-sm flex items-center justify-center gap-2 text-white shadow-xl shadow-teal-500/20 transition-all active:scale-[0.99] border border-white/10">
        ...
        <div className="absolute inset-0 bg-black/70 backdrop-blur-md z-[150] flex items-center justify-center p-4">
          <div className="bg-gray-900/90 border border-white/10 backdrop-blur-2xl rounded-3xl p-5 w-full max-w-xs flex flex-col gap-4 shadow-2xl">
            ...
            <div className="flex flex-col gap-2">
              <button onClick={() => executeSave(currentMelodyId ? 'overwrite' : 'new')} className="w-full py-3 bg-blue-600 text-xs font-black rounded-2xl text-white shadow-lg active:scale-95 transition-all">保存する</button>
      ...
      {isListOpen && (
        <div className="absolute inset-0 bg-black/70 backdrop-blur-md z-[100] flex flex-col animate-in fade-in duration-200 lg:hidden">
          <div className="w-full max-w-[430px] h-[80vh] my-auto mx-auto bg-gray-900/90 border border-white/10 backdrop-blur-2xl rounded-3xl p-5 flex flex-col shadow-2xl">
            <div className="flex items-center justify-between pb-3 border-b border-white/10">
              ...
              <button onClick={() => setIsListOpen(false)} className="w-8 h-8 bg-white/10 rounded-full flex items-center justify-center text-gray-400 hover:text-white transition-colors"><X size={16} /></button>
            </div>
            <div className="flex-1 overflow-y-auto py-4 flex flex-col gap-3 scrollbar-none">
              {savedMelodies.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-gray-500"><Music size={40} className="mb-2 opacity-20" /><p className="text-xs font-bold">まだ保存されたデータがありません</p></div>
              ) : (
                savedMelodies.map((item) => (
                  <div key={item.id} onClick={() => handleLoadMelody(item)} className="p-4 bg-gray-950/80 border border-white/5 hover:border-teal-500/40 rounded-2xl flex items-center justify-between cursor-pointer active:scale-[0.98] transition-all">
                    <div className="flex flex-col gap-1">
                      <h4 className="text-sm font-black text-white">{item.title}</h4>
                      <p className="text-[10px] text-gray-500 font-mono uppercase">{item.savedAt}</p>
                    </div>
                    <Trash2 size={16} className="text-gray-500 hover:text-red-400 transition-colors" onClick={(e) => { e.stopPropagation(); const next = savedMelodies.filter(m => m.id !== item.id); setSavedMelodies(next); localStorage.setItem('easyComposer_saved_melodies', JSON.stringify(next)); }} />
                  </div>
                ))
              )}
            </div>
